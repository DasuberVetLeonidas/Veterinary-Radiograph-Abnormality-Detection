...
wandb.init(project="MURA Finetune Ver.3",config=hyperparameter_defaults)
...
print("Class weights between abnormal and normal is {}".format(class_weights))
...
class NewPad(object):
    def __init__(self, fill=128, padding_mode='edge'):
...
def train_model(model, dataloaders, criterion, optimizer,
                num_epochs, log_path, model_save_path,scheduler,
                img_counter, current_step, wbconfig=wbconfig):
    print(f'Training log is saved to {log_path}')
    with open(log_path,"w") as file:
        file.write("\n")
    since = time.time()
    total_steps = len(dataloaders['train'].dataset)* 1 //wbconfig.batch_size * wbconfig.num_epochs
    val_acc_history = []
    train_acc_history = []
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                if phase == 'val':
                    ...
                    wandb.log({'Validation Example':[wandb.Image(image,caption="Validation Example from Animal Dataset")]})
                if phase == 'train':
                    current_step += 1
                    ...
                    if current_step % 70 == 0:
                        img_counter+=1
                        image = ToPILImage(inputs[0])
                        wandb.log({str(img_counter):[wandb.Image(image,caption="Training Example from Animal Dataset")]})
    ...
    return model, val_acc_history,train_acc_history,current_step,img_counter
...
for cv in np.arange(5,6):
    ...
    models[cv].load_state_dict(torch.load('/media/dasleo/LEOHDD/models/AnimalRadV2/MURA2/14.h5',
                                map_location='cuda:0'))
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=700, gamma=0.5)
    num_epochs = wbconfig.num_epochs
    log_path = './MURA_Anim_Finetune_Ver3/log/CV'+str(cv)+'_log.txt'
    model_save_path = './MURA_Anim_Finetune_Ver3/CV'+str(cv)
    
    '''Step 4: Train model'''
    trained_model,val_acc_history,train_acc_history,current_step,img_counter  = train_model(models[cv],
                                                                   dataloaders_dict, 
                                                                   criterion,
                                                                   optimizer,
                                                                   num_epochs,
    ...
    save_path = './MURA_Anim_Finetune_Ver3/CV'+str(cv)+'/best.h5'
    ...
    fig = plt.legend()
    fig_save_path = './MURA_Anim_Finetune_Ver3/log/CV'+str(cv)+'_Train_Val_Log.png'
    ...
    report_save_path = './MURA_Anim_Finetune_Ver3/log/CV'+str(cv)+'_Report.txt'
